...
namespace forest {
        ...
        struct binary_search_tree_node {
                 ...
                 */
                binary_search_tree_node(const key_t key) {
                        this->key = key;
                        this->parent.reset();
                        this->left = nullptr;
                        this->right = nullptr;
        ...
        private:
                std::shared_ptr<binary_search_tree_node <key_t> > root;
                void pre_order_traversal(std::shared_ptr<binary_search_tree_node <key_t> > &x, void handler(std::shared_ptr<binary_search_tree_node <key_t> >)) {
                        if (x == nullptr) return;
                        handler(x);
                        pre_order_traversal(x->left, handler);
                        pre_order_traversal(x->right, handler);
                }
                void in_order_traversal(std::shared_ptr<binary_search_tree_node <key_t> > &x, void handler(std::shared_ptr<binary_search_tree_node <key_t> >)) {
                        if (x == nullptr) return;
                        in_order_traversal(x->left, handler);
                        handler(x);
                        in_order_traversal(x->right, handler);
                }
                void post_order_traversal(std::shared_ptr<binary_search_tree_node <key_t> > &x, void handler(std::shared_ptr<binary_search_tree_node <key_t> >)) {
                        if (x == nullptr) return;
                        post_order_traversal(x->left, handler);
                        post_order_traversal(x->right, handler);
                        handler(x);
                }
                void breadth_first_traversal(std::shared_ptr<binary_search_tree_node <key_t> > &x, void handler(std::shared_ptr<binary_search_tree_node <key_t> >)) {
                        std::queue <std::shared_ptr<binary_search_tree_node <key_t> > > queue;
                        if (x == nullptr) return;
                        queue.push(x);
                        while(queue.empty() == false) {
                                std::shared_ptr<binary_search_tree_node <key_t> > y = queue.front();
                                handler(y);
                                queue.pop();
                ...
                const unsigned long long height(std::shared_ptr<binary_search_tree_node <key_t> > &x) {
                        if (x == nullptr) return 0;
                        return std::max(height(x->left), height(x->right)) + 1;
                }
                const unsigned long long size(std::shared_ptr<binary_search_tree_node <key_t> > &x) {
                        if (x == nullptr) return 0;
                        return size(x->left) + size(x->right) + 1;
                }
                void graphviz(std::ofstream &file, std::shared_ptr<binary_search_tree_node <key_t> > &x, unsigned long long *count) {
                        if (x == nullptr) return;
        ...
        public:
                 ...
                 * @return void
                 */
                void pre_order_traversal(void handler(std::shared_ptr<binary_search_tree_node <key_t> >)) {
                 ...
                 * @return void
                 */
                void in_order_traversal(void handler(std::shared_ptr<binary_search_tree_node <key_t> >)) {
                 ...
                 * @return void
                 */
                void post_order_traversal(void handler(std::shared_ptr<binary_search_tree_node <key_t> >)) {
                 ...
                 * @return void
                 */
                void breadth_first_traversal(void handler(std::shared_ptr<binary_search_tree_node <key_t> >)) {
                 ...
                 */
                const std::shared_ptr<binary_search_tree_node <key_t> > insert(const key_t key) {
                        std::shared_ptr<binary_search_tree_node <key_t> > current = root;
                        std::shared_ptr<binary_search_tree_node <key_t> > parent = nullptr;
                        ...
                        current = std::make_shared<binary_search_tree_node <key_t> >(key);
                 ...
                 */
                const std::shared_ptr<binary_search_tree_node <key_t> > search(const key_t key) {
                        std::shared_ptr<binary_search_tree_node <key_t> > x = root;
                        while (x != nullptr) {
                                if (key > x->key) {
                                        x = x->right;
                                } else if (key < x->key) {
                 ...
                 */
                const std::shared_ptr<binary_search_tree_node <key_t> > minimum() {
                        std::shared_ptr<binary_search_tree_node <key_t> > x = root;
                 ...
                 */
                const std::shared_ptr<binary_search_tree_node <key_t> > maximum() {
                        std::shared_ptr<binary_search_tree_node <key_t> > x = root;
                        if (x == nullptr) return nullptr;
                 ...
                 */
                const std::shared_ptr<binary_search_tree_node <key_t> > successor(const key_t key) {
                        std::shared_ptr<binary_search_tree_node <key_t> > x = root;
                        while (x != nullptr) {
                                if (key > x->key) {
                                        x = x->right;
                                        ...
                                        std::shared_ptr<binary_search_tree_node <key_t> > parent = x->parent.lock();
                 ...
                 */
                const std::shared_ptr<binary_search_tree_node <key_t> > predecessor(const key_t key) {
                        std::shared_ptr<binary_search_tree_node <key_t> > x = root;
                        while (x != nullptr) {
                                if (key > x->key) {
                                        x = x->right;
