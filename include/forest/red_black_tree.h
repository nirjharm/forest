...
namespace forest {
        ...
        private:
                std::shared_ptr<red_black_tree_node <key_t> > root;
                void pre_order_traversal(std::shared_ptr<red_black_tree_node <key_t> > &x, void handler(std::shared_ptr<red_black_tree_node <key_t> >)) {
                        if (x == nullptr) return;
                        handler(x);
                        pre_order_traversal(x->left, handler);
                        pre_order_traversal(x->right, handler);
                }
                void in_order_traversal(std::shared_ptr<red_black_tree_node <key_t> > &x, void handler(std::shared_ptr<red_black_tree_node <key_t> >)) {
                        if (x == nullptr) return;
                        in_order_traversal(x->left, handler);
                        handler(x);
                        in_order_traversal(x->right, handler);
                }
                void post_order_traversal(std::shared_ptr<red_black_tree_node <key_t> > &x, void handler(std::shared_ptr<red_black_tree_node <key_t> >)) {
                        if (x == nullptr) return;
                        post_order_traversal(x->left, handler);
                        post_order_traversal(x->right, handler);
                        handler(x);
                }
                void breadth_first_traversal(std::shared_ptr<red_black_tree_node <key_t> > &x, void handler(std::shared_ptr<red_black_tree_node <key_t> >)) {
                        std::queue <std::shared_ptr<red_black_tree_node <key_t> > > queue;
                        if (x == nullptr) return;
                        queue.push(x);
                        while(queue.empty() == false) {
                                std::shared_ptr<red_black_tree_node <key_t> > y = queue.front();
                                handler(y);
                                queue.pop();
                ...
                void graphviz(std::ofstream &file, std::shared_ptr<red_black_tree_node <key_t> > &x, unsigned long long *count) {
                        if (x == nullptr) return;
                ...
                void left_rotate(std::shared_ptr<red_black_tree_node <key_t> > &x) {
                        std::shared_ptr<red_black_tree_node <key_t> > y = x->right;
                        if(y != nullptr) {
                                x->right = y->left;
                ...
                void right_rotate(std::shared_ptr<red_black_tree_node <key_t> > &x) {
                        std::shared_ptr<red_black_tree_node <key_t> > y = x->left;
                        if (y != nullptr) {
                                x->left = y->right;
                ...
                std::shared_ptr<red_black_tree_node <key_t> > find_sibling(std::shared_ptr<red_black_tree_node <key_t> > &x) {
                ...
                std::shared_ptr<red_black_tree_node <key_t> > find_parent(std::shared_ptr<red_black_tree_node <key_t> > &x) {
                        return x->parent.lock();
                }
                std::shared_ptr<red_black_tree_node <key_t> > find_grand_parent(std::shared_ptr<red_black_tree_node <key_t> > &x) {
                        if (find_parent(x) != nullptr) {
                                return find_parent(x)->parent.lock();
                        }
                        return nullptr;
                }
                std::shared_ptr<red_black_tree_node <key_t> > find_uncle(std::shared_ptr<red_black_tree_node <key_t> > &x) {
                        if (find_grand_parent(x) != nullptr) {
                                return find_sibling(find_parent(x));
                        }
                        return nullptr;
                }
                void fix(std::shared_ptr<red_black_tree_node <key_t> > &x) {
                        std::shared_ptr<red_black_tree_node <key_t> > parent = nullptr;
                        std::shared_ptr<red_black_tree_node <key_t> > grand_parent = nullptr;
                        while ((x != root) && (x->color != black) && (x->parent.lock()->color == red)) {
                                ...
                                if (parent == grand_parent->left) {
                                        std::shared_ptr<red_black_tree_node <key_t> > uncle = grand_parent->right;
                                        /**
                                         ...
                                         */
                                        std::shared_ptr<red_black_tree_node <key_t> > uncle = grand_parent->left;
                                        /**
        ...
        public:
                ...
                const std::shared_ptr<red_black_tree_node <key_t> > insert(const key_t key) {
                        ...
                        current = std::make_shared<red_black_tree_node <key_t> > (key, red);
                 ...
                 */
                const std::shared_ptr<red_black_tree_node <key_t> > minimum() {
                        std::shared_ptr<red_black_tree_node <key_t> > x = root;
                 ...
                 */
                const std::shared_ptr<red_black_tree_node <key_t> > maximum() {
                        std::shared_ptr<red_black_tree_node <key_t> > x = root;
                        if (x == nullptr) return nullptr;
                ...
                const std::shared_ptr<red_black_tree_node <key_t> > successor(const key_t key) {
                        ...
                        while (x != nullptr) {
                                if (key > x->key) {
                                        ...
                                        std::shared_ptr<red_black_tree_node <key_t> > parent = x->parent.lock();
